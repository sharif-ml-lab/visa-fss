...
class FewShotSegMultiQ(nn.Module):
    """
    ALPNet
    Args:
    ...
    """
    def __init__(self, in_channels=3, FG_PROT_MODE='gridconv+', BG_PROT_MODE='gridconv', pretrained_path=None, cfg=None):
        super(FewShotSegMultiQ, self).__init__()
        self.pretrained_path = pretrained_path
        self.config = cfg or {'align': False}
        self.FG_PROT_MODE = FG_PROT_MODE
        self.BG_PROT_MODE = BG_PROT_MODE
    ...
    def get_encoder(self, in_channels):
        ...
        if self.config['which_model'] == 'dlfcn_res101':
            ...
            self.encoder = TVDeeplabRes101Encoder(use_coco_init, multiq=True)
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs, isval, val_wsize, show_viz = False):
        ...
        # print(n_ways, n_shots, n_queries)
        ...
        # print(sup_bsize, img_size, qry_bsize)
        ...
        # supp_fts = img_fts[:n_ways * n_shots * sup_bsize].view(n_ways, n_shots, sup_bsize, -1, *fts_size)  # Wa x Sh x B x C x H' x W'
        supp_fts = img_fts[:1, 256:512].view(n_ways, n_shots, sup_bsize, -1, *fts_size)
        # print(supp_fts.shape)
        qry_fts = img_fts[1:].view(3, 1, -1, *fts_size)   # N x B x C x H' x W'
        # print(qry_fts.shape)
        # exit()
        ...
        # print(supp_fts.shape, qry_fts.shape, fore_mask.shape, back_mask.shape)
        ...
        for epi in range(1): # batch dimension, fixed to 1
            ...
            _raw_score, _, aux_attr = self.cls_unit(qry_fts, supp_fts, res_bg_msk, mode = self.BG_PROT_MODE, thresh = BG_THRESH, isval = isval, val_wsize = val_wsize, vis_sim = show_viz  )
            ...
            for way, _msk in enumerate(res_fg_msk):
                _raw_score, _, aux_attr = self.cls_unit(qry_fts, supp_fts, _msk.unsqueeze(0), mode = self.FG_PROT_MODE if F.avg_pool2d(_msk, 4).max() >= FG_THRESH and self.FG_PROT_MODE != 'mask' else 'mask', thresh = FG_THRESH, isval = isval, val_wsize = val_wsize, vis_sim = show_viz  )
        ...
        return output, align_loss / sup_bsize, [bg_sim_maps, fg_sim_maps], assign_maps, qry_fts
    ...
    def alignLoss(self, qry_fts, pred, supp_fts, fore_mask, back_mask):
        ...
        for way in range(n_ways):
            ...
            for shot in range(n_shots):
                ...
                _raw_score_bg, _, _ = self.cls_unit(qry = img_fts, sup_x = qry_fts, sup_y = qry_pred_bg_msk.unsqueeze(-3), mode = self.BG_PROT_MODE, thresh = BG_THRESH )

                scores.append(_raw_score_bg)

                _raw_score_fg, _, _ = self.cls_unit(qry = img_fts, sup_x = qry_fts, sup_y = qry_pred_fg_msk.unsqueeze(-3), mode = self.FG_PROT_MODE if F.avg_pool2d(qry_pred_fg_msk, 4).max() >= FG_THRESH and self.FG_PROT_MODE != 'mask' else 'mask', thresh = FG_THRESH )
