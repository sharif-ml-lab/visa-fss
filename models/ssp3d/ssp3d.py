...
class SSP3D_MatchingNet(nn.Module):
    def __init__(self, backbone, refine_steps=1, average=False):
        super(SSP3D_MatchingNet, self).__init__()
        ...
        print(f'SSP3D: refine steps: {self.refine_steps}, average: {self.average}')
    ...
    def forward(self, img_s_list, mask_s_list, img_q, mask_q):
        ...
        for rf in range(self.refine_steps):
            ...
            SSFP, SSBP, ASFP, ASBP = self.SSP_func(feature_q, current_out)
            if (self.average) and ((rf + 1) < self.refine_steps):
                SSFP = SSFP.mean(0).unsqueeze(0)
            FP = FP * 0.5 + SSFP * 0.5
            BP = SSBP * 0.3 + ASBP * 0.7
