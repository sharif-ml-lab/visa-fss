...
from dataloaders.SSLRegGeomDataset2 import SSLRegGeomDataset2
...
def main(_run, _config, _log):
    ...
    tr_parent = SSLRegGeomDataset2( # base dataset
    ...
    log_loss = {'loss': 0, 'align_loss': 0, 'fg_loss': 0}
    ...
    for sub_epoch in range(n_sub_epoches):
        ...
        for batch_index, sample_batched in enumerate(trainloader):
            ...
            if _config['fg_loss_coeff'] > 0 and sub_epoch >= 0:
                fg_loss = _config['fg_loss_coeff'] * fg_size_loss(query_pred)
            else:
                fg_loss = 0

            loss = query_loss + align_loss + fg_loss
            loss.backward()
            optimizer.step()
            scheduler.step()
            ...
            if (i_iter + 1) % _config['print_interval'] == 0:
                ...
                print(f'step {i_iter+1}: loss: {loss}, align_loss: {align_loss}, fg_loss: {fg_loss}')
            ...
            #     val = evaluate_prop(_run, _config, _log, model, reg_model) 
            #     _log.info('###### Taking snapshot ######')
            #     torch.save(model.state_dict(),
            #         os.path.join(f'{_run.observers[0].dir}/snapshots', f'{i_iter + 1}_{val:.4f}.pth'))

            if (i_iter - 2) > _config['n_steps']:
                return 1 # finish up

            if (i_iter + 1) % _config['validate_every'] == 0:
                val = evaluate_prop(_run, _config, _log, model, reg_model) 
                if val > best_validation:
                    best_validation = val
                    _log.info('###### Taking snapshot ######')
                    torch.save(model.state_dict(), os.path.join(f'{_run.observers[0].dir}/snapshots', f'{i_iter + 1}_{val:.4f}.pth'))
