...
class SSLRegGeomDataset2(BaseDataset):
    def __init__(self, which_dataset, base_dir, idx_split, mode, transforms, scan_per_load, min_fg = '',
        ...
        """
        super(SSLRegGeomDataset2, self).__init__(base_dir)
        self.img_modality = DATASET_INFO[which_dataset]['MODALITY']
        self.sep = DATASET_INFO[which_dataset]['_SEP']
        self.label_name = DATASET_INFO[which_dataset]['PSEU_LABEL_NAME']
        ...
        self.scan_num_slices = []
        self.ordered_scan_keys = list(sorted(self.scans.keys()))
        self.total_slices = 0
        for k in self.ordered_scan_keys:
            self.scan_num_slices.append(self.total_slices + self.scans[k].shape[-1])
            self.total_slices += self.scans[k].shape[-1]
        print(self.scan_num_slices)
        print(self.ordered_scan_keys)
    ...
    def set_random_order(self):
        self.order = torch.randperm(self.total_slices)

    def get_vol_slice(self, slice_num):
        # slice_num = self.order[index]
        i = 0
        while i < len(self.scan_num_slices):
            if slice_num < self.scan_num_slices[i]:
                vol = self.ordered_scan_keys[i]
                prev = 0 if i == 0 else self.scan_num_slices[i-1]
                slc = slice_num - prev
                return vol, slc
            i += 1
    ...
    def __getitem__(self, index):
        ...
        slice_num = torch.randint(0, self.total_slices, size=(1,)).item()
        v, s = self.get_vol_slice(slice_num)
        ...
        volume = self.scans[v]
        v_mask = self.labels[v]
        num_slices = volume.shape[-1]

        # Choose support and queries
        # random_center_index = torch.randint(0, num_slices, size=(1,)).item()
        random_center_index = s
        candidate_indices = torch.arange(max(0, random_center_index-self.query_range), min(random_center_index+self.query_range+1, num_slices))
